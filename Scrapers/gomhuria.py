...
OUTPUT_PREFIX = S3_PREFIX  # where scraped dumps go

MAX_PAGES_PER_SECTION = int(os.getenv("MAX_PAGES_PER_SECTION", "3"))  # starter number, editable later
MAX_ARTICLES_PER_SECTION = int(os.getenv("MAX_ARTICLES_PER_SECTION", "6"))
REQUEST_TIMEOUT = int(os.getenv("REQUEST_TIMEOUT", "30"))
PAGE_SLEEP = float(os.getenv("PAGE_SLEEP", "1.0"))
ARTICLE_SLEEP_MIN = float(os.getenv("ARTICLE_SLEEP_MIN", "0.8"))
ARTICLE_SLEEP_MAX = float(os.getenv("ARTICLE_SLEEP_MAX", "1.6"))
...
SECTIONS = {
    ...
    # Above URL sometimes has encoded segments; we will also allow using the two links you provided:
    # Provided links from user:
...
# ---------------- UTILITIES ----------------
ARTICLE_URL_RE = re.compile(r"/Gomhuria/\d+\.html$")  # accept only urls ending with /Gomhuria/<digits>.html
...
def is_valid_article_url(href: str) -> bool:
    ...
    if not parsed.path:
        return False
    # Only accept patterns like /Gomhuria/<digits>.html
    path = parsed.path
...
def init_driver():
    ...
    # recommended to reduce detection
...
def merge_detail_blocks(soup: BeautifulSoup) -> str:
    ...
    """
    # Remove obvious junk nodes globally
    ...
    for d in detail_divs:
        # Remove nested nodes that are clearly noise inside the block
    ...
    # Remove any remnants of typical unwanted repeated phrases
    for p in JUNK_PHRASES:
        full = full.replace(p, " ")
        ...
        try:
            ...
            for href in article_links:
                ...
                if href in seen_links:
                    ...
                    continue
                # simple safeguard: skip if url path contains non-digit article id
                ...
                if not article_data:
                    ...
                    seen_links.add(href)  # mark as seen anyway to avoid reattempts
                    continue

                title = article_data.get("title") or ""
                # prefer title from listing if missing
                if not title:
                    # try to extract title from the link text on listing
            ...
            # if we reached per-section quota, stop
            if collected >= MAX_ARTICLES_PER_SECTION:
            ...
            # continue to next page (resilient)
            continue
...
def main():
    ...
    try:
        ...
        for name, template in SECTIONS.items():
            ...
            # small random pause between sections
