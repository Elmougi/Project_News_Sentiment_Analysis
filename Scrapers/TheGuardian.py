...
# ---------------------------
# Helper: normalize a Guardian article to your unified schema
# ---------------------------
def normalize_guardian_item(item):
    """
    Turn a Guardian API item (with show-fields=all) into the project's schema,
    keeping the original Guardian abstract but renaming it to "summary".
    """
    fields = item.get("fields", {}) or {}

    # Guardian "abstract" is usually trailText or standfirst
    summary = (
    ...
    # Full text (prefer plain text version)
...
def fetch_guardian_articles(sections=None, page_size=50, max_pages=5, order_by="newest", keep_fields=True):
    """
    sections: list of sections to fetch, or None for all.
    page_size: number of items per page (max 200 for Guardian API)
    max_pages: how many pages to fetch per section (safety cap)
    order_by: 'newest' or 'relevance'
    keep_fields: if False, will not request show-fields=all (not recommended)
